...
class PandasChain:
    ...
    def __commit_block(self,block): 
        # change block status to commit
        block.set_status('COMMITED')
        # obtain the merkle
        merkle_root_hash = block.get_simple_merkle_root()
        # generate and set the block hash
        new_hash = Block(self.__seq_id, merkle_root_hash)
        # set prev_hash to the previous block hash
        self.__prev_hash = block
    ...
    def get_values(self):
        return_list = []
        for block in self.__chain:
            return_list = return_list + block.get_values().to_list()
...
class Block:
    ...
    def display_header(self): 
        print_list = {
            ...
            'prev_block_hash': self.__prev_hash,
            'merkle_hash': self.__merkle_tx_hash,
            '# transactions': len(self.__transactions)
        }

        print_str = ''
        for key in print_list.keys:
            print_str = print_str + f'{key}: {print_list[key]}, '
    ...
    def add_transaction(self,s,r,v): 
        ...
        df_row = [[ts, s, r, v, tx_hash]]
        new_transaction = pd.DataFrame(df_row, columns=self.__col_names)# Create DataFrame with transaction data (a DataFrame with only 1 row)
    ...
    def get_simple_merkle_root(self): 
        merkle_str = ''
        for hash in self.__transactions['TxHash'].to_list():
            merkle_str = merkle_str + hash
